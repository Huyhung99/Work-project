jQuery(function () {
  ...
  $(document).ready(function () {
    ...
    if ($('#admin-menu').length > 0) {
      $('nav.navbar.navbar-expand-lg.navbar-light.bg-blue').css('margin-top', '40px');
    }

    const tableRelateImage = $('[id^="edit-field-anh-lien-quan-und"]');
    if (tableRelateImage.length > 0) {
      setInterval(function () {
        const delete_table = $('[id^="edit-field-anh-lien-quan-und"] td input[name^="field_anh_lien_quan_und"]');
        delete_table.each(function () {
          $(this).val('X')
        });
        let valueTitle = $('#san-pham-node-form #edit-title').val();
        $('input[id^="edit-field-anh-lien-quan-und"][type="text"]').each(function(){
          $(this).val(valueTitle);
        });
      }, 1000)
    }
    if($('#admin-menu').length > 0){
      $('.post-date-1').removeClass('d-none');
      $('#edit-field-banner').removeClass('d-none');
      $('#edit-field-hien-thi-o-trang-chu').removeClass('d-none');
    }
  });
});
