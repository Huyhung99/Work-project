...
(function ($) {
	Drupal.behaviors.mymodule = {
        ...
        $(document).on('click','.tacdong .fa-trash',function (e){
          e.preventDefault();
          ...
          var id_xoa_hocphi=$(this).parents('tr').find('.tacdong .fa-edit').attr('data-value');
          $.ajax({
            url: 'https://mamnonlaxanh.com/sua-hoc-phi',
            data:
              {
                id_xoa_hocphi : id_xoa_hocphi,
              },
            dataType: 'json',
            type: 'post',
            success: function(data){
              ...
              if(data.thongbao!=='')
              {
                $('.load_du_lieu').html('<p>'+data.thongbao+'</p>')
              }
              else
              {
                setTimeout(function (){location.reload()}, 1000);
          ...
          if( lophoc && nam && thang)
          {
            $.ajax({
              ...
              success: function(data){
                ...
                if(data.str_tong!=='')
                {
                  $('.thongbao_hoc_phi').append(data.str_tong);
                }
              },
              error: function(r1, r2){
                consoloe.log(r1);
        ...
        $(document).on('click','.table-diem-danh-suat-an thead tr td',function () {
          if($(this).hasClass('click_day'))
          {
            ...
            trmot.each(function () {
              var chaytieptuc=0;
              var tdbu=$(this).children('td');
              tdbu.each(function () {
                chaytieptuc++;
                if(chaytieptuc===demvitri)
                {
                  // $('.myCheckbox').prop('checked', false);
                  if($(this).find('input').hasClass('da_them'))
                  {
                    $(this).children('input').prop('checked', false).attr("checked",false).removeClass('da_them');
                  }
                }
              })
            })
          ...
          else
          {
            ...
            trmot.each(function () {
              ...
              tdbu.each(function () {
                ...
                if(chaytieptuc===demvitri)
                {
                  if(!($(this).children('input').attr("checked") && $(this).children('input').attr("checked")==='checked'))
                  {
                    $(this).children('input').addClass('da_them').prop('checked', true).attr("checked","checked");
                  }
                }
              })
            })
        ...
        $(document).on('click','.dong-du-lieu',function (e) {
          e.preventDefault();
          var tiendonghoc=$(this).parents('tr').find('.gia_tri_tien_hoc').text().replace('.','').replace(',','').trim();
          $(this).parents('tr').find('.nhap_tien').val(parseInt(tiendonghoc.replace('.','')));
        })
        ...
        $(document).on('click','.huy_bo_giao_vien',function (e) {
          e.preventDefault();
          $(this).parents('tr').remove();
          $('.danh_sach_giao_vien tr').each(function () {
            if($(this).hasClass('d-none'))
            {
              $(this).removeClass('d-none');
            }
        ...
        $(document).on('click','.huy_bo_phu_huynh',function (e) {
          e.preventDefault();
          $(this).parents('tr').remove();
          $('.danh_sach_phu_huynh tr').each(function () {
            if($(this).hasClass('d-none'))
            {
              $(this).removeClass('d-none');
            }
        ...
        $(document).on('click', '.them-dong-phu-huynh', function (e){
          ...
          <tr class="form-them-phu-huynh ktra">
            <td></td>
            <td><input type="text" name="hotenph" class="hoten_themph form-control" placeholder="Tên bố">
            <input type="text" name="tenme" class="tenme_themph form-control" placeholder="Tên mẹ"></td>
            <td><input type="text" name="sodienThoai" class="sodienThoai_themph form-control"></td>
            <td><input type="email" name="email" class="email_themph form-control" placeholder="Có thể không điền"></td>
            <td></td>
        ...
        $(document).on('click','.luu_them_phu_huynh',function (e) {
          ...
          var tenme_themph=$(this).parents('tr').find('.tenme_themph').val();
            ...
            $.ajax({
              ...
              success: function(data){
                ...
                if(data.kiemchung==='khong')
                {
                  ...
                  $('.thongbaobanghi').html('Đã có bản ghi này').after(' - <span><a href="#" nid_xoa="'+data.nid_xoa+'" class="xoa_diem_danh_xuat_an">Xóa bản ghi</a></span>');
                }
                if($('.luu_table_diem_danh_suat_an').hasClass('d-none'))
                {
                  $('.luu_table_diem_danh_suat_an').removeClass('d-none');
        ...
        //   if(su_kientinh==0)
        //   {
        //     $('.thongbaobanghi').after(' - <span></span>')
        //   }
        //   else
        //   {
        ...
        $(document).on('click', '.xoa_diem_danh_xuat_an', function (e){
          e.preventDefault();
