...
function getMenuLeMinhLand(){
    ...
    $str = "";
    $ds_chung_cu_cao_cap='<ul class="sub-menu">';
    $danh_sach_chung_cu_cao_cap=node_load_multiple(array(),array('type'=>'du_an'));
    ...
    foreach ($mainMenu as $item) {
        if ($item['link']['hidden'] == 0) {
          ...
          if (count($item['below']) == 0) {
            if(trim($item['link']['link_title'])=='Căn hộ dịch vụ' || trim($item['link']['link_title'])=='Nhà và biệt thự' || trim($item['link']['link_title'])=='Văn phòng cho thuê' || trim($item['link']['link_title'])=='Chung cư cao cấp')
            {
          ...
          if(trim($item['link']['link_title'])=='Căn hộ dịch vụ' || trim($item['link']['link_title'])=='Nhà và biệt thự' || trim($item['link']['link_title'])=='Văn phòng cho thuê' || trim($item['link']['link_title'])=='Chung cư cao cấp')
          {
            if(trim($item['link']['link_title'])=='Căn hộ dịch vụ')
            {
              ...
              else
              {
                ...
                {
                  if(trim($item['link']['link_title'])=='Chung cư cao cấp')
                  {
                    $str=$str.$ds_chung_cu_cao_cap;
                  }
                }
              }
            }
...
{
  $tat_ca_khu_vuc=node_load_multiple(array(), array('type'=>'du_an'));
  $str='';
  foreach ($tat_ca_khu_vuc as $id_node => $khu_vuc)
  {
    $ten=$khu_vuc->title;
    $path=check_plain(url('node/'.$id_node, array()));
    $str=$str.'<li><a href="'.$path.'">Căn hộ tại '.$ten.'</a></li>';
  }
  $str='<ul>'.$str.'</ul>';
  return $str;
}
...
function form_sidebar(){
  ...
  $str_select_3='';
  foreach ($treeBath as $item)
  {
      $str_select_3=$str_select_3.'<option value="'.$item->tid.'">'.$item->name.'</option>';
  }
